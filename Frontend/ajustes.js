...
document.addEventListener('DOMContentLoaded', async () => {
            ...
            if (res.ok) {
                ...
                if (listaTarjetas) {
                    ...
                    if (tarjetas.length === 0) {
                        listaTarjetas.innerHTML = '<tr><td colspan="3" style="text-align: center; color: var(--text-muted);">No tienes métodos de pago guardados.</td></tr>';
                        return;
                    }
                    tarjetas.forEach(t => {
                        const tr = document.createElement('tr');
                        tr.innerHTML = `<td><span style="letter-spacing: 2px; font-family: monospace; font-size: 1.1rem; color: var(--primary-slate);">${t.enmascarada}</span></td><td>${t.fecha}</td><td><button class="btn-eliminar-tarjeta" data-id="${t.id_metodo}" style="background-color: var(--danger-color); color: white; border: none; padding: 0.3rem 0.5rem; border-radius: 4px; cursor: pointer; font-weight: bold;">X</button></td>`;
                        listaTarjetas.appendChild(tr);
                    });
                }
            }
    ...
    if (listaTarjetas) {
        listaTarjetas.addEventListener('click', async (e) => {
            if (e.target.classList.contains('btn-eliminar-tarjeta')) {
                if (!confirm('¿Estás seguro de que deseas eliminar este método de pago guardado?')) return;
                
                const idMetodo = e.target.getAttribute('data-id');
                showSpinner();
                try {
                    const response = await fetch(`/api/metodos-pago/${idMetodo}`, {
                        method: 'DELETE',
                        headers: { 'Authorization': `Bearer ${token}` }
                    });
                    if (response.ok) {
                        ...
                        const data = await response.json();
                        showToast(data.error || 'Error al eliminar la tarjeta.', 'error');
                    }
                } catch (error) {
                    console.error(error);
    ...
    if (btnGodMode) {
        btnGodMode.addEventListener('click', async () => {
            if (!confirm('¿Activar God Mode para esta cuenta?')) return;
            showSpinner();
            try {
                const res = await fetch('/api/usuarios/godmode', {
                    method: 'POST',
                    headers: { 'Authorization': `Bearer ${token}` }
                });
                if (res.ok) {
                    localStorage.setItem('isGodMode', 'true');
                    showToast('God Mode activado. Redirigiendo...', 'success');
                    setTimeout(() => window.location.href = 'dashboard.html', 1500);
                } else {
                    showToast('Error al activar God Mode', 'error');
                }
